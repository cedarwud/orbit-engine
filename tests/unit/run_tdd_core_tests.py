...
class TDDTestRunner:
    ...
    def run_quick_smoke_test(self):
        ...
        smoke_tests = [
            # Phase 1: SGP4核心測試
            "tests/unit/algorithms/test_sgp4_orbital_engine.py",
            # Phase 2: 信號品質核心測試
            "tests/unit/algorithms/test_signal_quality_calculator.py",
            "tests/unit/algorithms/test_satellite_visibility_filter.py",
            # Phase 3: Stage4-5核心測試
            "tests/unit/algorithms/test_timeseries_preprocessing.py",
            "tests/unit/algorithms/test_data_integration_engine.py"
        ]
        ...
        for test in smoke_tests:
            test_name = test.split('/')[-1].replace('.py', '')
            print(f"🧪 {test_name}...")
...
使用示例:
  ...
  python scripts/run_tdd_core_tests.py --smoke
    ...
    parser.add_argument(
        "--component", "-c",
        choices=["sgp4", "signal", "visibility", "stage4", "stage5", "all"],
        default="all",
