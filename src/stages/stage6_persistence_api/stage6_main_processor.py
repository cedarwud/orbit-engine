...
class Stage6PersistenceProcessor(BaseStageProcessor):
    ...
    def execute(self, input_data: Any) -> Dict[str, Any]:
        """執行Stage 6持久化與API處理 - 統一接口方法"""
        result = self.process(input_data)
        if result.status == ProcessingStatus.SUCCESS:
            # 保存結果到檔案
            try:
                output_path = self.save_results(result.data)
                self.logger.info(f"✅ Stage 6結果已保存至: {output_path}")
            except Exception as e:
                self.logger.warning(f"⚠️ Stage 6結果保存失敗: {e}")

            # 保存驗證快照
            try:
                snapshot_success = self.save_validation_snapshot(result.data)
                if snapshot_success:
                    self.logger.info("✅ Stage 6驗證快照保存成功")
            except Exception as e:
                self.logger.warning(f"⚠️ Stage 6驗證快照保存失敗: {e}")

            return result.data
        else:
            # 從錯誤列表中提取第一個錯誤訊息，如果沒有則使用狀態
            error_msg = result.errors[0] if result.errors else f"處理狀態: {result.status}"
            raise Exception(f"Stage 6 處理失敗: {error_msg}")
    ...
    def save_validation_snapshot(self, processing_results: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            snapshot_data = {
                ...
                'processing_summary': {
                    'services_initialized': len(processing_results.get('service_status', {})),
                    'api_endpoints_created': len(processing_results.get('api_endpoints', [])),
                    'websocket_endpoints_created': len(processing_results.get('websocket_endpoints', [])),
                    'data_persistence_operations': processing_results.get('statistics', {}).get('data_persistence_operations', 0),
                    'processing_time': processing_results.get('statistics', {}).get('processing_time_seconds', 0.0),
                    'processing_status': 'completed'
                },
                'service_metrics': {
                    'api_response_time_ms': processing_results.get('performance_metrics', {}).get('api_response_time_ms', 0),
                    'cache_hit_rate': processing_results.get('performance_metrics', {}).get('cache_hit_rate', 0.0),
                    'active_connections': processing_results.get('performance_metrics', {}).get('active_connections', 0),
                    'storage_usage_gb': processing_results.get('performance_metrics', {}).get('storage_usage_gb', 0.0),
                    'architecture_version': 'v2.0_persistence_api'
                },
                'compliance_status': {
                    'documentation_compliance': processing_results.get('metadata', {}).get('compliance_status', 'UNKNOWN'),
                    'api_standards': 'REST_GraphQL_WebSocket',
                    'persistence_strategy': 'multilayer_cache_filesystem'
                },
            ...
            # 保存快照
            snapshot_path = validation_dir / "stage6_validation.json"
            with open(snapshot_path, 'w', encoding='utf-8') as f:
                json.dump(snapshot_data, f, indent=2, ensure_ascii=False, default=str)

            self.logger.info(f"✅ Stage 6驗證快照已保存至: {snapshot_path}")
            return True

        except Exception as e:
            self.logger.error(f"❌ Stage 6驗證快照保存失敗: {e}")
            return False
