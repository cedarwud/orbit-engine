...
╠════════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
║  本驗證器採用 **分級 Fail-Fast 原則**，區分核心要求和品質要求：         ║
║                                                                            ║
║  ┌─────────────────────────────────────────────────────────────────────┐  ║
║  │ 🔴 核心要求（CRITICAL - 失敗則停止執行）                            │  ║
║  ├─────────────────────────────────────────────────────────────────────┤  ║
║  │ 1. Epoch 獨立性檢查 (independent_epochs_check)                      │  ║
║  │    - 標準: ≥30% epoch 多樣性（或小數據集≥3個不同epoch）            │  ║
║  │    - 依據: Vallado 2013 - 每顆衛星必須使用其 TLE 的獨立 epoch     │  ║
║  │    - 違反影響: 軌道傳播系統性誤差，學術標準不合規                  │  ║
║  │                                                                       │  ║
║  │ 2. 時間一致性檢查 (timestamp_consistency_check)                     │  ║
║  │    - 標準: 時間戳記與 epoch 差距 ≤7 天                             │  ║
║  │    - 依據: Vallado 2013 Section 8.6 - SGP4 精度範圍                │  ║
║  │    - 違反影響: SGP4 傳播精度劣化                                   │  ║
║  └─────────────────────────────────────────────────────────────────────┘  ║
║                                                                            ║
║  ┌─────────────────────────────────────────────────────────────────────┐  ║
...
║  ├─────────────────────────────────────────────────────────────────────┤  ║
║  │ 3. Epoch 分布檢查 (distribution_check)                              │  ║
║  │    - 標準: Epoch 跨度 >72 小時（3天）                               │  ║
║  │    - 依據: Space-Track.org TLE 更新頻率統計                         │  ║
║  │    - 違反影響: 數據可能來自單一更新批次，但不違反學術標準          │  ║
║  │                                                                       │  ║
║  │ 🔍 常見觸發場景:                                                     │  ║
║  │    - Stage 1 使用 latest_date 篩選（僅保留單日數據）               │  ║
║  │    - 刻意選擇特定時間窗口進行研究                                   │  ║
║  │                                                                       │  ║
║  │ ✅ 設計預期行為（不是 BUG）:                                         │  ║
║  │    當 Epoch 跨度 <72h 但獨立性 ≥30% 時，會顯示:                    │  ║
║  │    「⚠️ Epoch 驗證有警告，但允許繼續處理」                          │  ║
║  │    這是正確的分級決策，因為核心學術要求（獨立性）已滿足            │  ║
║  └─────────────────────────────────────────────────────────────────────┘  ║
║                                                                            ║
║  ⚙️  決策流程（stage4_link_feasibility_processor.py:736-748）:          ║
║                                                                            ║
║  if overall_status == 'FAIL':                                             ║
║      if independent_epochs_check.independent_epochs == False:             ║
║          → ERROR: "Epoch 獨立性驗證失敗" → 停止執行                       ║
║      else:                                                                 ║
║          → WARNING: "Epoch 驗證有警告，但允許繼續處理" → 繼續執行         ║
║                                                                            ║
╚════════════════════════════════════════════════════════════════════════════╝
