...
⚠️ **v3.0 架構變更說明**:
- **可見性篩選已移至 Stage 4.1**（原「第一層快速篩選」）
- **Stage 3 專注純座標轉換**（處理全部衛星，無篩選）

#### 1. **座標轉換架構** (v3.0)

**Stage 3 核心職責**: Skyfield 專業級 TEME→WGS84 座標轉換

- **處理範圍**: 全部衛星（9,041 顆，無預篩選）
  - **輸入**: Stage 2 的 TEME 軌道狀態時間序列
  - **方法**: 完整 Skyfield IAU 標準算法（亞米級精度）
  - **處理規模**:
    - **Starlink**: 8,390顆 × 191時間點 (95分鐘軌道週期)
    - **OneWeb**: 651顆 × 218時間點 (112分鐘軌道週期)
    - **總計**: ~1,726,294 個座標點的精密轉換
  - **輸出**: 所有衛星的完整 WGS84 座標時間序列

- **與 Stage 4 的職責分工**:
  - **Stage 3**: 純座標轉換（處理全部衛星）→ 提供完整 WGS84 數據
  - **Stage 4.1**: 可見性篩選（9,041→~2,000顆）→ 星座感知門檻判斷
  - **效率優化**: Stage 4 篩選後，Stage 5/6 只需處理 ~2,000 顆候選衛星
