...
3. **Epoch 選擇策略**:
   - **衛星池規劃**: 最新日期 ±24h 容差（確保 SGP4 精度 1-3km，保留 99.8% 數據）
   - **科學依據**: SGP4 誤差增長率 1-3km/天，48h 窗口內精度優秀
   - 註: 強化學習訓練為未來獨立工作（見下文「未來工作」章節）
4. **分析方法**: 固定時間點的完整軌道週期分析 (離線計算)
...
### 🔮 未來工作：強化學習換手優化 (獨立於六階段外實作)

> **注意**: 本節為未來研究方向規劃，將在獨立模塊 `tools/ml_training_data_generator/` 中實作，不屬於當前六階段核心流程。
...
10. **訓練數據生成** (規劃中):
    ...
    - **依賴**: 需要 Stage 6 的 3GPP 事件檢測輸出作為訓練數據來源
...
- 🔄 **3GPP 事件檢測量化目標** (Stage 6 實測數據 - 2025-10-10優化後):
  - **事件數量**: 1,250-2,500 個 3GPP 事件/軌道週期
    - 計算基礎: 126 衛星 × 220 時間點 = 27,720 檢測機會
    - **實測結果**: **4,695 個事件** ✅
      - A3事件: **678個** (相對比較，邻居优于服务+offset)
      - A4事件: **2,519個** (绝对门槛，邻居优于-100 dBm)
      - A5事件: **1,217個** (双门槛，NTN优化阈值-41/-34 dBm)
      - D2事件: **281個** (距离换手，动态阈值)
  - **事件频率归一化**: 0.31 次/卫星/分钟 = 1次/3.2分钟
    - 文献对比: ✅ 符合LEO换手频率（2-5分钟/次）
    - Starlink实测对比: ✅ 30秒间隔 vs Starlink 15秒更新（更保守）
  - **時間覆蓋率**: 100% (220/220 時間點完整遍歷)
  - **參與衛星**: 126 顆 (100% 衛星參與事件檢測)
  - **驗證標準**: 生產級門檻 1,250 事件 (已達成 **376% 目標** ✅)
- 🔮 **RL訓練數據** (未來工作): 基於真實池狀態變化和 3GPP 事件的豐富訓練樣本
...
```
數據流程:
@data/tle_data/ → 選定時間點 → 各星座獨立軌道週期計算 → 池狀態分析 → 3GPP事件檢測
              → (未來) RL訓練數據生成 (獨立模塊)
