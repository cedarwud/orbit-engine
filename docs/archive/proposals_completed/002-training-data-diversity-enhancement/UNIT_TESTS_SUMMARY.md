...
| 測試文件數 | 3 |
| 測試類數 | 11 |
| 測試用例數 | 17 (Markov) + 20 (Loo) + 19 (Simulator) = **56** |
| 功能覆蓋 | 3 核心模組 |
...
# test_propagation_simulator.py
Ran 19 tests in 0.006s
OK ✅
```

### 測試修正記錄
1. **API 方法名稱對齊**: 修正測試使用 `adjust_for_elevation()` 和 `get_expected_state_duration()`
2. **隨機成分處理**: 對包含隨機成分的測試（LOS/多徑），改為驗證合理範圍而非嚴格比較
3. **環境依賴**: 所有測試在 venv 環境中成功運行（需 astropy）

### 注意事項
- ⚠️ 完整整合測試（Stage 5 pipeline）需要在實際運行環境中驗證
- ✅ 所有核心模組功能已充分測試並通過
